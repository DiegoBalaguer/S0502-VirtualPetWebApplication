...
import com.virtualgame.entites.petUser.dto.PetUserFullDto;
...
public class PetUserControllerAdmin {
    ...
    @PostMapping("/create")
    public ResponseEntity<PetUserFullDto> create(
            @RequestBody @Valid PetUserCreateDto createDto) {

        PetUserFullDto createdPet = petUserServiceImpl.createPetUser(createDto, currentUserService.getCurrentUserId());
    ...
    @GetMapping("/find/{id}")
    public ResponseEntity<PetUserFullDto> findPetById(@PathVariable Long id) {
        PetUserFullDto pet = petUserServiceImpl.findPetUserById(id);
    ...
    @GetMapping("/list")
    public ResponseEntity<List<PetUserFullDto>> findAllPets() {
        List<PetUserFullDto> pets = petUserServiceImpl.findAllPetsUser();
    ...
    public ResponseEntity<PetUserFullDto> updatePet(
            @PathVariable Long id,
            @RequestBody @Valid PetUserFullDto fullDto) {

        PetUserFullDto updatedPet = petUserServiceImpl.updatePetUser(id, fullDto, currentUserService.getCurrentUserId());
    ...
    public ResponseEntity<List<PetUserFullDto>> findPetsByType(
            @RequestParam Long typeId) {

        List<PetUserFullDto> pets = petUserServiceImpl.findPetsUserByType(typeId);
        return ResponseEntity.ok(pets);
    }

    // TODO: Falta ver como implementar el cambio de habitat, si como un punto a parte o dentro del update del pet
    // TODO: falta ver como implementar las acciones del pet. Entendemos que es pasar a una accion X tiempo y despues volver a CHILLING (que no varia las variables).
}